...
uint64_t cb_hash_fnv1a64(const void *data, size_t len) {
    if (data == NULL || len == 0) return FNV1A64_OFFSET_BASIS;
...
uint32_t cb_hash_fnv1a32(const void *data, size_t len) {
    if (data == NULL || len == 0) return FNV1A32_OFFSET_BASIS;
...
/* CRC-32 (IEEE polynomial) - static const table, no lazy init needed */
