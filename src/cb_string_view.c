...
CB_StringView cb_sv_trim_ascii(CB_StringView sv) {
    if (sv.data == NULL || sv.len == 0) return sv;
...
int cb_sv_starts_with(CB_StringView sv, CB_StringView prefix) {
    if (prefix.len > sv.len) return 0;
    if (sv.data == NULL || prefix.data == NULL) return 0;
    return memcmp(sv.data, prefix.data, prefix.len) == 0;
}

int cb_sv_ends_with(CB_StringView sv, CB_StringView suffix) {
    if (suffix.len > sv.len) return 0;
    if (sv.data == NULL || suffix.data == NULL) return 0;
...
int cb_sv_find(CB_StringView sv, CB_StringView needle, size_t *out_pos) {
    /* Empty needle is never found */
    if (needle.len == 0) return 0;
    if (needle.len > sv.len) return 0;
    if (sv.data == NULL || needle.data == NULL) return 0;
    for (size_t i = 0; i <= sv.len - needle.len; i++) {
        if (memcmp(sv.data + i, needle.data, needle.len) == 0) {
            if (out_pos != NULL) *out_pos = i;
            return 1;
        }
    }
    return 0;
...
CB_Error cb_sv_to_i64(CB_StringView sv, int64_t *out_value) {
    if (out_value == NULL) return CB_ERR_INVALID_ARGUMENT;
    if (sv.data == NULL || sv.len == 0) return CB_ERR_PARSE;
    CB_StringView trimmed = cb_sv_trim_ascii(sv);
    if (trimmed.len == 0) return CB_ERR_PARSE;
    char *endptr = NULL;
    char *copy = (char *)malloc(trimmed.len + 1);
    if (copy == NULL) return CB_ERR_OUT_OF_MEMORY;
...
int cb_sv_split_next(CB_SplitIter *iter, CB_StringView *out_part) {
    if (iter == NULL || out_part == NULL) return 0;
    if (iter->input.data == NULL) return 0;
    if (iter->offset > iter->input.len) return 0;
